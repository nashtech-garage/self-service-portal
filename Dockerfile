...
COPY --chown=node:node packages/backend/org/rbac-policy.csv packages/backend/org/rbac-policy.csv
COPY --chown=node:node packages/backend/templates/ packages/backend/templates/
RUN ls . packages/ packages/backend/ packages/backend/org/
...
CMD ["node", "packages/backend", "--config", "app-config.container.yaml"]
